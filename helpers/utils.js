...
module.exports = utils = {
    ...
    generateLoanPayment: (loanInfo) => {
        return new Promise(async (resolve, reject) => {
            try {
                const { duration, totalRemain, createdBy, interest, store } = loanInfo
                ...
                await session.withTransaction(async () => {
                    for (let i = 0; i < durationTime; i++) {
                        ...
                        const loanPayment = await LoanPayment.create({ ...loanItem, createdBy, store, loan: loanId, dueDate: paymentTime, interestAmount, totalAmount, principalBalance })
